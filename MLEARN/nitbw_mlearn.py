...
class MLearn:
    ...
    def vergleiche(self, test_data, label_namen=None):
        ...
        # Logistische Regression (nur fuer binaere Labels 0/1)
        try:
            labels = set([label for _, label in self.data])
            if labels.issubset(set([0.0, 1.0])):
                self.train_logreg()
                acc = self.accuracy(test_data, self.predict_logreg)
                ergebnisse["Logistische Regression"] = acc
            else:
                ergebnisse["Logistische Regression"] = "Nur fuer binaere Labels (0/1)"
        except Exception as e:
            ergebnisse["Logistische Regression"] = "Fehler: {}".format(e)
